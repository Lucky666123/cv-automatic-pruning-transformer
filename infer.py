...
def main():
    ...
    model.load_state_dict(torch.load(os.path.join(args.output_dir, "1_%s_checkpoint.bin" % args.name)))
    # model.load_state_dict(torch.load(os.path.join(args.output_dir, "0_%s_checkpoint.bin" % args.name)))
    # Infer
    infer(args, model, clip_layer_id, stream_id=1, infer_mode=True, threshold=0.5)
